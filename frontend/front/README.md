...
> 基于 **Vue 3 + Vite + TypeScript + Pinia + Vue Router + Axios + Element Plus** 的毕业设计演示前端。  
> 支持图像分割、视频实时预览、视频二阶段最终生成（realtime -> finalize -> result）、模型切换、分辨率切换。

## 1. 项目定位

本项目是《基于卷积神经网络的自动驾驶图像语义分割系统设计与实现》的前端展示与联调工程，重点能力：

- 图像上传并调用语义分割接口，展示原图/分割图/融合图与类别占比。
- 视频上传后先进行**实时预览推理**（WebSocket 推进度、FPS、预览帧）。
- 实时阶段完成后自动触发**最终视频生成**，并轮询获取最终可播放视频地址。
- 支持模型切换（DeepLabV3+ ResNet50 / BiSeNetV2）。
- 支持输入分辨率切换（由后端 `/api/segment/resolutions` 提供选项）。
